...
def write_batch(batch, out_path, compression: str | None = None):
    with WSSink(out_path, compression=compression) as sink:
        for row in batch:
            sink.write(row)
...
def shard_from_audio_dir(
    ...
    def flush_batch():
        ...
        write_batch(batch, out_path, compression=None)
        shard_idx += 1
        batch = []
